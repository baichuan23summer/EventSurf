...
async function recommendEvents(text) {
  ...
  try {
    ...
    // Get matched events from GPT
    const matchedIndices = await getRecommendationsFromGPT(text, eventData);
    ...
    if (matchedIndices.length === 0) {
      ...
      matchedIndices.forEach((index) => {
        ...
        li.innerHTML = `<a href="${eventData.url[index]}" target="_blank">${eventData.name[index]}</a>
                        <p>Date: ${eventData.date[index]}</p>
                        <p>Location: ${eventData.location[index]}</p>`;
